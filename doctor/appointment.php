...
<body>
                            ...
                            <?php
                                ...
                                for ( $x=0; $x<$result->num_rows;$x++){
                                    ...
                                    echo '<tr >
                                        ...
                                        <button id="complaintsBtn" class="btn-primary-soft btn" onclick="loadComplaints()" style="padding-left: 40px;padding-top: 12px;padding-bottom: 12px;margin-top: 10px;"><font class="tn-in-text">Complaints</button>
                                        <button
                                        id="reportBtn"
                                        class="btn-primary-soft btn"
                                       
                                        onclick="openReportPopup(this)"
                                        style="padding-left: 40px;padding-top: 12px;padding-bottom: 12px;margin-top: 10px;"><font class="tn-in-text">
                                        Report
                                    </button>
    ...
    function loadComplaints() {
        // Use AJAX to fetch patient complaints
        // Replace 'your_php_script.php' with the actual PHP script to retrieve complaints.
        // You'll need to pass the selected patient's name to the PHP script.

        const pname = prompt("Enter patient name:"); // Prompt user for patient name
        if (pname) {
            // Make an AJAX request to fetch complaints for the given patient
            const xhr = new XMLHttpRequest();
            xhr.open("POST", "your_php_script.php", true);
            xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    // Handle the response here, e.g., display complaints in a popup
                    alert("Complaints for " + pname + ": " + xhr.responseText);
                }
            };
            // Send the pname to the PHP script
    ...
    if (pname !== null) {
        // Prompt for diagnosis and prescription
        const diagnosis = prompt("Enter diagnosis:");
        const prescription = prompt("Enter prescription:");

        if (diagnosis !== null && prescription !== null) {
            // Use AJAX to send pname, diagnosis, and prescription to PHP script
            const xhr = new XMLHttpRequest();
            xhr.open("POST", "your_report_php_script.php", true);
            xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4 && xhr.status === 200) {
